...
query <- LoadFileInput(path = "count.rds")
...
# you set in the app
cells.use <- query[["nCount_RNA", drop = TRUE]] <= 50000 &
  query[["nCount_RNA", drop = TRUE]] >= 502 &
  query[["nFeature_RNA", drop = TRUE]] <= 7000 &
...
if ("percent.mt" %in% c(colnames(x = query[[]]))) {
  cells.use <- query[["percent.mt", drop = TRUE]] <= 15 &
